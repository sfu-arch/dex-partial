#!/bin/bash
###############################################################################
# QW2: Original CHIME Zipfian Skew Sweep — Node 1 (Compute Node)
#
...
RESULTS_DIR="$SCRIPT_DIR/results/chime_original"
...
wait_for_iteration() {
    ...
    echo ">>> [sync] Waiting for node0 to signal iteration $expected_iter (qw2_orig_iter key)..."
    local attempts=0
    local max_attempts=120
    while [ $attempts -lt $max_attempts ]; do
        if nc -z -w 2 "$MEMC_IP" "$MEMC_PORT" 2>/dev/null; then
            local val
            val=$(printf "get qw2_orig_iter\r\nquit\r\n" | nc -w 2 "$MEMC_IP" "$MEMC_PORT" 2>/dev/null | grep -A1 "^VALUE" | tail -1 | tr -d '\r')
            if [ "$val" = "$expected_iter" ]; then
                echo ">>> [sync] OK — qw2_orig_iter=$expected_iter (node0 ready)"
                # Wait for node0's BINARY to register
            ...
            [ -n "$val" ] && echo ">>> [sync] qw2_orig_iter='$val' (waiting for $expected_iter)..."
        fi
        attempts=$((attempts + 1))
...
echo ""
echo "============================================================"
echo "  QW2: ORIGINAL CHIME Zipfian Skew Sweep — Node 1 (Compute)"
...
for config in "${SKEW_CONFIGS[@]}"; do
    ...
    echo ""
    echo "============================================================"
    echo "  ORIGINAL CHIME Compute — $LABEL (uniform=$UNIFORM, theta=$ZIPF_THETA)"
    echo "============================================================"
    ...
    # Run benchmark
    cd "$CHIME_DIR/build"
    echo ">>> Running ORIGINAL CHIME compute for $LABEL ..."
    ...
    sudo ./latency_bench \
        ...
        2>&1 | tee "$RESULTS_DIR/original_${LABEL}_stdout.log"
    
    # Collect results
    for f in chime_read_latency.dat chime_range_latency.dat; do
        if [ -f "$f" ]; then
            cp "$f" "$RESULTS_DIR/original_${LABEL}_${f}"
            echo ">>> Saved $RESULTS_DIR/original_${LABEL}_${f}"
        fi
    done
...
echo ""
echo "============================================================"
echo "  QW2: ORIGINAL CHIME SWEEP COMPLETE — Node 1 (Compute)"
